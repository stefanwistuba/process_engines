...
class FireworkEngine:
    def __init__(self, output_path, input_file_path):
    ...
    def execute(self, name):
        try:
            ...
            for idx, command in enumerate(self.commands):
                if idx > 0:
                    tasks.append(Firework(ScriptTask.from_str(command), name=f'task_{idx}', fw_id=idx, parents=[tasks[idx-1]]))
                else:
                    tasks.append(Firework(ScriptTask.from_str(command), name=f'task_{idx}', fw_id=idx))
...
parser.add_argument('-wf', metavar='Workflow Path', required=False, default=None)
...
if __name__ == '__main__':
    ...
    if args.wf != None:
        fp = FireworkEngine(args.p, args.wf)
